...
def joern_create(
    ...
    joern_process.communicate(timeout=180)
...
def generate_original_dataset(df: pd.DataFrame, output_path: str, workers: int = 4):
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as executor, Progress() as progress:
        ...
        futures = {executor.submit(process_single_example, index, row): index for index, row in pending_rows}
        ...
        for future in as_completed(futures):
            ...
            progress.update(
                main_task,
                advance=1,
                description=f"[magenta]Generating ORIGINAL dataset ({done}/{len(pending_rows)})...",
            )
...
if __name__ == "__main__":
    ...
    for dataset, dataset_df in cwe20cfa_datasets.items():
        ...
        final_df = generate_original_dataset(filtered_df, out_path, workers=args.workers)
