...
def joern_create(joern_path, in_path, out_path, cpg_files):
    ...
    for cpg_file in cpg_files:
        ...
        if os.path.exists(in_path+cpg_file):
            ...
            delete_project_cmd = f'delete("{cpg_file}")\r'.encode()
            print(f"joern> {import_cpg_cmd.decode()}")
            joern_process.stdin.write(import_cpg_cmd)
            print(f"joern> {run_script_cmd.decode()}")
            joern_process.stdin.write(run_script_cmd)
            print(f"joern> {delete_project_cmd.decode()}")
            joern_process.stdin.write(delete_project_cmd)
