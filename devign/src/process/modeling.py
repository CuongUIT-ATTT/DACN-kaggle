...
class Train(object):
    def __init__(self, step, epochs, verbose=True):
        ...
        self.best_val_acc = 0.0 # Initialize a variable to store the best accuracy observed
        ...
        for epoch in range(self.epochs):
            ...
            if val_loader_step is not None:
                ...
                if val_stats.acc() > self.best_val_acc:
                    self.best_val_acc = val_stats.acc()  # Update best validation accuracy
                    self.step.save()  # Save model weights
                    print(f"Epoch {epoch+1}; Validation accuracy improved to {val_stats.acc():.4f}. Model Saved.")
